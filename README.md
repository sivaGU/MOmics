...
`Single Omics & Integration/single_omics_proteomics_DEG.R`, `Single Omics & Integration/single_omics_proteomics_KEGG.R`, and `Single Omics & Integration/diabolo_integration.R` still reference the original authors' local file paths (e.g. `~/Desktop/...`) rather than `data/discovery/`, so they need path updates before they'll run against this repo's data.
